...
if(is_array($_GET) && count($_GET)>0 && isset($_GET["next"]) && isset($_GET["jiang"]))//先判断是否通过get传值了
{
	...
	if($jiang == 'zzh'){
		//总排行
		$sqlz = "select b.openid ,b.nickname ,b.headimgurl,a.praised_num from event_user a,wx_user b where a.state='3' and b.id=a.wx_id order by a.praised_num desc limit {$next_num},5;";
		//echo $sqlz;
		$re = select_DB_2($sqlz);
		$zz['meg'] = 'OK';
		$zz['info'] = $re;
